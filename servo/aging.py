...
def mdeg_send2(ser, id, mdeg, mvel):
    head = bytes([0x3E, 0xA4, id, 0x0C])
    data1 = (int(mdeg * 1000.0)).to_bytes(8, 'little', signed=True)
    data2 = (int(mvel * 1000.0)).to_bytes(4, 'little', signed=True)
    data = data1 + data2
    msg = head + checksum(head).to_bytes(1, 'little') + data + checksum(data).to_bytes(1, 'little')
    str=''.join(f' {byte:02x}' for byte in msg)
    print(str)
...
for id in ids:
  i = id - 10
  mdeg_send2(ser, id, pos0[i], 800)
  # mdeg_send(ser, id, pos0[i])
  time.sleep(0.001)
...
for id in ids:
  i = id - 10
  mdeg_send2(ser, id, pos1[i], 800)
  # mdeg_send(ser, id, pos1[i])
  time.sleep(0.001)
...
for id in ids:
  i = id - 10
  mdeg_send2(ser, id, pos2[i], 800)
  # mdeg_send(ser, id, pos2[i])
  time.sleep(0.001)
