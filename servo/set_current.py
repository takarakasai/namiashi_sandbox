...
def mcur_send(ser, id, mdeg):
    head = bytes([0x3E, 0x95, id, 0x04])
    data = (int(mdeg * 1000.0)).to_bytes(4, 'little', signed=True)
    msg = head + checksum(head).to_bytes(1, 'little') + data + checksum(data).to_bytes(1, 'little')
    str=''.join(f' {byte:02x}' for byte in msg)
    print(str)
    ser.write(msg)
...
time.sleep(0.1)
for id in ids:
    pos = inits[id] * ratio[id] * direction[id]
    print("set {} >>:{} ({}, {}, {})".format(id, pos, inits[id], ratio[id], direction[id]))
    # mcur_send(ser, id, pos)
    mcur_send(ser, id, 10)
    time.sleep(0.3)
    mcur_send(ser, id, 10)
    time.sleep(0.3)
